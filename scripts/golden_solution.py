...
COMMIT_SHA = "27f7f75569cf51b7ee5a731a9fb468020f770451"
...
def _apply_file(repo_root: Path, spec: FileSpec) -> FileResult:
	...
	if not target.exists():
		if spec.create_if_missing is None:
			return FileResult(spec.path, "error", "target file does not exist")

		content = spec.create_if_missing
		missing_after = [marker for marker in spec.after_markers if marker not in content]
		if missing_after:
			return FileResult(
				spec.path,
				"mismatch",
				f"post-check failed, missing {len(missing_after)} after marker(s) in new file",
			)

		target.parent.mkdir(parents=True, exist_ok=True)
		_atomic_write(target, content)
		patch = _make_unified_diff(spec.path, "", content)
		return FileResult(spec.path, "created", "file created", patch)
...
def _build_specs() -> tuple[FileSpec, ...]:
	test_using_before = """using Microsoft.Extensions.Logging;

namespace HomeBase.Core.Tests.Chat;"""

	test_using_after = """using Microsoft.Extensions.Logging;
using Microsoft.Extensions.DependencyInjection;

namespace HomeBase.Core.Tests.Chat;"""

	test_constructor_instance1_before = """        var store = new SqliteConversationStore(Path.Combine(_tempDirectory, "homebase.db"));
	...
	test_constructor_instance1_after = """        var store = new SqliteConversationStore(Path.Combine(_tempDirectory, "homebase.db"));
        var fileDocumentService = new FileDocumentService(Path.Combine(_tempDirectory, "documents"), loggerFactory);
        var serviceProvider = new ServiceCollection().BuildServiceProvider();
        _service = new LocalHostConversationService(fileDocumentService, settings, store, loggerFactory, serviceProvider);"""

	test_constructor_instance2_before = """        var sut = new LocalHostConversationService(
	...
	test_constructor_instance2_after = """        var serviceProvider = new ServiceCollection()
	...
	test_constructor_instance3_before = """        var fileDocumentService = new FileDocumentService(documentRoot, loggerFactory);
        var service = new LocalHostConversationService(fileDocumentService, settings, store, loggerFactory, httpClient);"""

	test_constructor_instance3_after = """        var fileDocumentService = new FileDocumentService(documentRoot, loggerFactory);
        var serviceProvider = new ServiceCollection()
            .AddSingleton(httpClient)
            .BuildServiceProvider();
        var service = new LocalHostConversationService(fileDocumentService, settings, store, loggerFactory, serviceProvider);"""
...
using Microsoft.Agents.AI;"""
	...
	local_service_field_before = """    private readonly ICustomLoggerFactory _loggerFactory;
    public LocalHostConversationService("""

	local_service_field_after = """    private readonly ICustomLoggerFactory _loggerFactory;
    private readonly IServiceProvider _serviceProvider;
    public LocalHostConversationService("""

	local_service_constructor_signature_before = """    public LocalHostConversationService(
	...
	local_service_constructor_signature_after = """    public LocalHostConversationService(
        IDocumentService fileDocumentService, 
        CoreSettings settings, 
        IConversationStore store,
        ICustomLoggerFactory loggerFactory,
	...
	local_service_constructor_before = """        _loggerFactory = loggerFactory;
	...
	local_service_constructor_after = """        _loggerFactory = loggerFactory;
...
"""
	...
	local_service_create_after = """    private ConversationState CreateConversation()
	{
		...
		var agent = new ChatClientAgent(
			chatClient: new OllamaClientWrapper(client, modelName),
			instructions: "You are a helpful assistant. Answer the user's questions based on the provided context.",
			name: "Terry",
			description: "A helpful assistant that provides information and answers questions based on the provided context.",
			tools: 
			[
				AIFunctionFactory.Create(DocumentTools.ListDocumentNames, nameof(DocumentTools.ListDocumentNames)),
				AIFunctionFactory.Create(DocumentTools.ReadDocument, nameof(DocumentTools.ReadDocument)),
			],
...
using Microsoft.Extensions.DependencyInjection;"""
	...
	program_services_before = """builder.Services.AddSingleton<IConversationService, LocalHostConversationService>();
...
builder.Services.AddSingleton(sp => new HttpClient
{
	...
	program_services_after = """builder.Services.AddSingleton<IConversationService, LocalHostConversationService>();
...
new HttpClient()
{
	...
	ollama_wrapper_content = """using System.Text.Json.Serialization;
using OllamaSharp;

public class OllamaClientWrapper : OllamaApiClient
{
    public OllamaClientWrapper(Configuration config) : base(config)
    {
    }

    public OllamaClientWrapper(string uriString, string defaultModel = "") : base(uriString, defaultModel)
    {
    }

    public OllamaClientWrapper(Uri uri, string defaultModel = "") : base(uri, defaultModel)
    {
    }

    public OllamaClientWrapper(HttpClient client, string defaultModel = "", JsonSerializerContext? jsonSerializerContext = null) : base(client, defaultModel, jsonSerializerContext)
    {
    }
}
"""
	...
	return (
		FileSpec(
			...
			transforms=(
				Transform("add_using_dependency_injection", test_using_before, test_using_after),
				Transform("update_test_constructor_1", test_constructor_instance1_before, test_constructor_instance1_after),
				Transform("update_test_constructor_2", test_constructor_instance2_before, test_constructor_instance2_after),
				Transform("update_test_constructor_3", test_constructor_instance3_before, test_constructor_instance3_after),
			),
			after_markers=(
				"using Microsoft.Extensions.DependencyInjection;",
		...
		FileSpec(
			...
			transforms=(
				...
				Transform("add_serviceprovider_field", local_service_field_before, local_service_field_after),
				Transform("update_constructor_signature", local_service_constructor_signature_before, local_service_constructor_signature_after),
			...
			after_markers=(
				...
				"public AgentSession Session { get; set; }",
				"new OllamaClientWrapper(client, modelName)",
			),
		),
		FileSpec(
			path="HomeBase.Core/Chat/OllamaWrapper.cs",
			transforms=(),
			after_markers=(
				"public class OllamaClientWrapper : OllamaApiClient",
				"public OllamaClientWrapper(HttpClient client,",
			),
			create_if_missing=ollama_wrapper_content,
		),
		FileSpec(
